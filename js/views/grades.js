...
const statusChip = (letter) => {
  ...
  if (st.k === 'pass') return '<span class="chip ok">Geçti</span>';
  if (st.k === 'cond') return '<span class="chip">Koşullu</span>';
  if (st.k === 'fail') return '<span class="chip bad">Kaldı</span>';
  return '';
};
...
function courseSummaryHtml(code) {
  ...
  if (calc.score !== null && !calc.done) {
    const targets = [['CC', 60], ['BB', 75], ['AA', 90]]
    ...
    html: `<div class="stack">
      ...
      <div class="card">
        ...
        <p class="tiny muted" style="margin:6px 0 0">
          ...
          Bu tablo mutlak değerlendirmedir. Bölüm bağıl değerlendirme uygularsa harfin farklı çıkabilir —
          o zaman dersin harfini sağ üstteki kutudan elle seç. Kesin bilgi OBS'dedir.
        </p>
      </div>
    </div>`,
