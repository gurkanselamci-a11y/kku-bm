...
const EMU = (() => {
  try {
    ...
    if (new URLSearchParams(location.search).has('emu')) sessionStorage.setItem('kkubm.emu', '1');
    return sessionStorage.getItem('kkubm.emu') === '1';
  } catch { return false; }
})();
...
export async function cloud() {
  ...
  const config = EMU
    ? { apiKey: 'demo-key', authDomain: 'localhost', projectId: 'demo-kkubm', appId: 'demo-app' }
    : firebaseConfig;
