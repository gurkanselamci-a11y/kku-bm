...
function kunye(ders, resmi, kapsam) {
  ...
  p.push('> Bu belge Kırıkkale Üniversitesi Mühendislik Fakültesi, Bilgisayar Mühendisliği');
  p.push('> Bölümü öğrencisi için hazırlanmış bir ders çalışma kaynağıdır.', '');
  ...
  else if (ders.resources?.length) p.push('', '**Kaynaklar.**', '', ders.resources.map((o) => `- ${duzMetin(o)}`).join('\n'));
  p.push('', '---', '');
  return p.join('\n');
}
...
Rolün: Bilgisayar Mühendisliği öğrencisine bu haftayı sıfırdan anlatan bir asistan.
Öğrenci konuyu hiç bilmiyor varsay; ama birinci sınıf matematiğini ve temel programlamayı
bildiğini varsayabilirsin.

Şu sırayı izle:
1. Konu neden var, hangi mühendislik problemini çözüyor? Somut bir örnekle başla.
2. Temel kavramları teker teker tanımla; her tanımdan sonra küçük bir örnek ver.
3. Formülleri yalnız söyleme, TÜRET ve terimlerin ne anlama geldiğini açıkla.
   Sayısal örneği adım adım, ara sonuçları söyleyerek çöz.
4. Kod varsa satır satır ne yaptığını anlat; çıktının neden o çıktı olduğunu göster.
5. Kaynakta "sık yapılan hatalar" ya da "dikkat" diye geçen yerleri mutlaka işle:
   yanlış olan nedir, doğrusu nedir, ayırt edici ölçüt nedir?
6. Sonunda 5 maddelik özet ver ve sınavda bu haftadan ne sorulabileceğini söyle.

Kurallar:
- Kaynakta olmayan bilgi ekleme; sayı, tarih ve karmaşıklık sınıfı uydurma.
- Terimleri Türkçe kullan, İngilizcesini ilk geçtiğinde parantez içinde ver.
